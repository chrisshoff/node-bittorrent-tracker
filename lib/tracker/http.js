...
function display(response, status, serverIp, serverPort) {
	var resp = "";
	resp += "Server IP: " + serverIp + ", Server Port: " + serverPort + "\n\n"; 
	for (var i in status) {
		resp += status[i].torrent + "\n";
		resp += "--------------\n";
		for (var j in status[i].peers) {
			resp += status[i].peers[j] + "\n";
		}
		resp += "\n";
	}
	...
	var server = http.createServer(function (request, response) {
		request.addListener('end', function() {
			...
			if (parts["pathname"] == "/status") {
				trackerInstance.getStatus(response, display, serverIp, serverPort);
			} else if (parts["pathname"] == "/server") {
				serverIp = query["ip"];
				...
				try {
					...
					// Get the torrent being announced by info_hash
					var file = trackerInstance.getFile(query["info_hash"]);
					// Create a peer based on ip address reported in the request, or a remote address if one wasn't provided
					var peer = tracker.Peer((query["ip"] || request.connection.remoteAddress), query["port"], query["left"]);
					...
					// Write the peer list, passing in the announcing peer's id to exclude it and the server ip and port to add
					var len = file.writePeers(peerBuffer, want, query["peer_id"], serverIp, serverPort);
