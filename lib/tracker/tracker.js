...
Peer.prototype = {
	...
	timedOut: function(n) {
		// Was the last time this peer was touched within the time of 2 announce requests?
...
File.prototype = {
	addPeer: function(peerId, peer, event) {
		...
		if (this.seeders + this.leechers < this.peerList.length / 2 && this.peerList.length > 10 || (n - this.lastCompact) > ANNOUNCE_INTERVAL * 2) {
			...
			// Look for peer in peer dictionary
		...
		if (this.peerDict.hasOwnProperty(peerId)) {
			...
			if (event == EVENT_STOPPED) {
				...
				// Touch the peer so it doesn't timeout
				oldPeer.touch()
			}
	...
	writePeers: function(b, count, peer_id, serverIp, serverPort) {
		var c = 0;
		if (count > this.seeders + this.leechers) {
			for (var i = this.peerList.length - 1; i >= 0; i--) {
				
				// Add the server in instead of the announcing peer
				if (i == this.peerDict[peer_id]) {
					if (serverIp != null && serverPort != null) {
						var p = Peer(serverIp, serverPort, 0);
					} else {
						continue;
					}
				} else {
			...
			for (var i = 0; i < m; i++) {
				...
				// Add the server in instead of the announcing peer
				if (index == this.peerDict[peer_id]) {
					if (serverIp != null && serverPort != null) {
						var p = Peer(serverIp, serverPort, 0);
					} else {
						continue;
...
Tracker.prototype = {
	...
	getStatus: function(response, display, serverIp, serverPort) {
		var status = {};
		var idx = 0;
		for (var i in this.files) {
			var infoHash = i;
			var torrent = this.files[i];
			var peers = torrent.peerDict;
			status[idx] = {};
			status[idx].peers = {};
			status[idx].torrent = idx + " | seeders: " + torrent.seeders + " | leechers: " + torrent.leechers;
			for (var j in torrent.peerList) {
				status[idx].peers[j] = torrent.peerList[j].ip;
			}
			
			idx++;
		}
		
		display(response, status, serverIp, serverPort);
	}
}
